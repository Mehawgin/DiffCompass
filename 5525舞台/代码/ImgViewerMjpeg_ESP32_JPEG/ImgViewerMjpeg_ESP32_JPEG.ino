 ...
 ******************************************************************************/
#define ROOT "/root"
#define MJPEG_FILENAME ROOT "/Mayday_Ashin.mjpeg"
#define MJPEG_OUTPUT_SIZE (240 * 240 * 2 / 4)          // memory for a output image frame
...
Arduino_GFX *gfx = new Arduino_ST7796(bus, TFT_RST, 1 /* rotation */);
...
//  if (!LittleFS.begin(false, ROOT))
  // if (!SPIFFS.begin(false, ROOT))
   SPI.begin(SCK /* CLK */, MISO /* D0/MISO */, MOSI/* CMD/MOSI */);
   if (!SD.begin(SD_CS))
...
//      uint16_t *output_buf = (uint16_t *)aligned_alloc(16, MJPEG_OUTPUT_SIZE);
      ...
      else
      {
        ...
        {
        	Serial.println(F("000000000000000000000"));
          while (mjpeg.readMjpegBuf())
          {
            // Read video
        	Serial.println(F("1111111111111111111111"));
        	...
        	Serial.println(F("33333333333333"));
