...
/*******************************************************************
 * @name       :void GUI_DrawPoint(uint16_t x,u16 y,uint16_t color)
 * @date       :2021-04-06 
...
{  	
	uint16_t i,j;			
	uint16_t width=ex-sx+1; 		//Get the width of the fill
	uint16_t height=ey-sy+1;		//Get the height of the fill
...
void LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	{  
		LCD_DrawPoint(uRow,uCol);//Draw a dot
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
void gui_circle(int xc, int yc,uint16_t c,int r, int fill)
{
	...
	{
		// If not filled (draw a hollow circle)
		while (x <= y) {
...
{  
	...
	LCD_SetWindows(x,y,x+size/2-1,y+size-1);//Setting up a single text display window
	if(!mode) //非叠加方式
	{		
		for(pos=0;pos<size;pos++)
		{
			if(size==12)temp=asc2_1206[num][pos];//Call 1206 font
			else temp=asc2_1608[num][pos];		 //Call 1608 font
			for(t=0;t<size/2;t++)
		    {                 
	...
	}else//叠加方式
	{
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {   
				POINT_COLOR=fc;              
		        if(temp&0x01)LCD_DrawPoint(x+t,y+pos);//Draw a dot    
		        temp>>=1; 
		    }
		}
	}
	POINT_COLOR=colortemp;	
	LCD_SetWindows(0,0,lcddev.width-1,lcddev.height-1);//The recovery window is full screen    	   	 	  
}
...
{         
    while((*p<='~')&&(*p>=' '))//Judge if it is illegal character!
    {   
...
void GUI_DrawFont16(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s,uint8_t mode)
{
	...
	uint16_t HZnum;
	uint16_t x0=x;
	HZnum=sizeof(tfont16)/sizeof(typFNT_GB16);	//Automatically count the number of Chinese characters
	...
	for (k=0;k<HZnum;k++) 
	{
	  ...
	  { 	LCD_SetWindows(x,y,x+16-1,y+16-1);
		...
		continue;  //Find the corresponding dot matrix font library and exit immediately to 
		           //prevent the influence of multiple Chinese characters repeated modulus
	}
...
void GUI_DrawFont24(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s,uint8_t mode)
{
	...
	uint16_t HZnum;
	uint16_t x0=x;
	HZnum=sizeof(tfont24)/sizeof(typFNT_GB24);	//Automatically count the number of Chinese characters
...
void GUI_DrawFont32(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s,uint8_t mode)
{
	...
	uint16_t HZnum;
	uint16_t x0=x;
	HZnum=sizeof(tfont32)/sizeof(typFNT_GB32);	//Automatically count the number of Chinese characters
	for (k=0;k<HZnum;k++) 
			{
        ...
        if(!bHz)
        {
	        ...
	        {          
		        ...
		        else
				{
					...
					x+=8; //Character, half of the whole word
					}
					else
					{
					LCD_ShowChar(x,y,fc,bc,*str,size,mode);
					x+=size/2; //Character, half of the whole word
					}
				} 
				str++; 
...
/*****************************************************************************
 * @name       :void Gui_Drawbmp16(uint16_t x,uint16_t y,uint16_t w, uint16_t h, const uint8_t *p)
 * @date       :2021-04-06 
 ...
 * @retvalue   :None
******************************************************************************/ 
void Gui_Drawbmp16(uint16_t x,uint16_t y,uint16_t w, uint16_t h, const uint8_t *p) 
{
  	int i; 
	uint8_t picH,picL; 
	LCD_SetWindows(x,y,x+w-1,y+h-1);//Window settings
    for(i=0;i<w*h;i++)
	{	
	 	picL=*(p+i*2);	//The data is at the top of the list
		picH=*(p+i*2+1);				
		Lcd_WriteData_16Bit(picH<<8|picL);  						
	}	
	LCD_SetWindows(0,0,lcddev.width-1,lcddev.height-1);//Restore the display window to full screen	
}
