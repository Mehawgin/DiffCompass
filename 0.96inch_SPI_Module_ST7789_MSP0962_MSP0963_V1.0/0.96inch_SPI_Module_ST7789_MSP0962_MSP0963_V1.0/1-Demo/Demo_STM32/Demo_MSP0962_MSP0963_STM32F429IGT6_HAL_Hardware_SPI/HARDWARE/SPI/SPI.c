...
void SPI_SetSpeed(uint8_t SpeedSet)
{
    ...
    SPI5_Handler.Instance->CR1 |= SpeedSet << 3;     /* 设置SPI速度 */
...
void SPI5_Init(void)	
{
    ...
    SPI5_Handler.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_2; /* 定义波特率预分频的值:波特率预分频值为2 */
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    GPIO_InitTypeDef  GPIO_InitStructure;
        ...
        __HAL_RCC_GPIOF_CLK_ENABLE(); /*GPIOA时钟使能 */
			__HAL_RCC_SPI5_CLK_ENABLE();  /* SPI5时钟使能 */
