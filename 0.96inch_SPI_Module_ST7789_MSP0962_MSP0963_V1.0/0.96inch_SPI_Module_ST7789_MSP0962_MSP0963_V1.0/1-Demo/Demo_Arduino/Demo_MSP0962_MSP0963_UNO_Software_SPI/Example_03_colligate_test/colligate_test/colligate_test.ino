...
LCDWIKI_SPI my_lcd(MODEL,CS,CD,-1,SDA,RST,SCK,LED); //model,cs,dc,sdo,sda,reset,sck,ledled
...
unsigned long show_triangle_function(void)
{
     ...
     my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width()/2-1, 0, my_lcd.Get_Display_Height());
     my_lcd.Draw_Fast_HLine(0, my_lcd.Get_Display_Height()/2-1, my_lcd.Get_Display_Width());
     for(i = 0;i <= my_lcd.Get_Display_Height()/2/10;i++)
     {
         my_lcd.Draw_Fast_HLine(my_lcd.Get_Display_Width()/2-1, my_lcd.Get_Display_Height()/2-1-i*10, 5);
         my_lcd.Draw_Fast_HLine(my_lcd.Get_Display_Width()/2-1, my_lcd.Get_Display_Height()/2-1+i*10, 5);
     }
     for(i = 1;i <= my_lcd.Get_Display_Width()/2/10;i++)
     {
         my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width()/2-1-i*10, my_lcd.Get_Display_Height()/2-1, 5);
         my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width()/2-1+i*10, my_lcd.Get_Display_Height()/2-1, 5);
     }
...
unsigned long show_fill_rectangle(void)
{
    ...
    for(i = 0;i<5;i++)
    {
          ...
          my_lcd.Fill_Rectangle(x_spec+i*side_len-1, 10+y_spec+i*side_len-1, x_spec+(i+1)*side_len-1, 10+y_spec+(i+1)*side_len-1);
          my_lcd.Fill_Rectangle(x_spec+i*side_len-1, 10+y_spec+(5-i)*side_len-1, x_spec+(i+1)*side_len-1, 10+y_spec+(4-i)*side_len-1); 
     }
     return micros()- time_start;   
}
...
unsigned long show_fill_round_rectangle(void)
{
    ...
    for(i = 0;i<5;i++)
    {
          ...
          my_lcd.Fill_Round_Rectangle(x_spec+i*side_len-1, 10+y_spec+i*side_len-1, x_spec+(i+1)*side_len-1, 10+y_spec+(i+1)*side_len-1,5);
          my_lcd.Fill_Round_Rectangle(x_spec+i*side_len-1, 10+y_spec+(5-i)*side_len-1, x_spec+(i+1)*side_len-1, 10+y_spec+(4-i)*side_len-1,5); 
     }
     return micros()- time_start;   
}
...
unsigned long show_fill_triangle(void)
{
    ...
    for(i = 0;i<5;i++)
    {
          ...
          my_lcd.Fill_Triangle(x_spec+i*side_len-1,10+y_spec+(i+1)*h_len-1,x_spec+side_len/2+i*side_len-1,10+y_spec+i*h_len-1,x_spec+(i+1)*side_len-1,10+y_spec+(i+1)*h_len-1); 
          my_lcd.Fill_Triangle(x_spec+i*side_len-1,10+y_spec+(5-i)*h_len-1,x_spec+side_len/2+i*side_len-1,10+y_spec+(4-i)*h_len-1,x_spec+(i+1)*side_len-1,10+y_spec+(5-i)*h_len-1);  
     }
     return micros()- time_start;   
}
...
unsigned long show_random_pixels(void)
{
    ...
    for(i = 0;i< 10000;i++)
    {
       ...
       my_lcd.Draw_Pixel(random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1));
    }
    return micros()- time_start; 
}
...
unsigned long show_random_lines(void)
{
    ...
    {
       my_lcd.Set_Draw_color(random(255),random(255),random(255));
       my_lcd.Draw_Line(random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1));
    }
    return micros()- time_start; 
}
...
unsigned long show_random_rectangles(void)
{
    ...
    {
       my_lcd.Set_Draw_color(random(255),random(255),random(255));
       my_lcd.Draw_Rectangle(random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1));
    }
    return micros()- time_start; 
}
...
unsigned long show_random_round_rectangles(void)
{
    ...
    {
       my_lcd.Set_Draw_color(random(255),random(255),random(255));
       my_lcd.Draw_Round_Rectangle(random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),5);
    }
    return micros()- time_start; 
}
...
unsigned long show_random_circles(void)
{
    ...
    {
       my_lcd.Set_Draw_color(random(255),random(255),random(255));
       my_lcd.Draw_Circle(40+random(my_lcd.Get_Display_Width()-80), 40+random(my_lcd.Get_Display_Height()-80), random(40));
    }
    return micros()- time_start; 
}
...
unsigned long show_random_triangles(void)
{
    ...
    {
       my_lcd.Set_Draw_color(random(255),random(255),random(255));
       my_lcd.Draw_Triangle(random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1),random(my_lcd.Get_Display_Width()-1),random(my_lcd.Get_Display_Height()-1));
    }
    return micros()- time_start; 
}
