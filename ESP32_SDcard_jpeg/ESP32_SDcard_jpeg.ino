...
void setup() {
  ...
  digitalWrite(25, HIGH); // TFT screen chip select
...
void loop() 
{
	...
	for (i = 1; i <= 273; i++)
	{
		snprintf(filename, sizeof(filename), "/240/hao%03d.jpg", i); 
...
void jpegRender(int xpos, int ypos)
{
	...
	{    
		...
		{
			snprintf(log, sizeof(log), "[PUSH]push fail!mcu_x[%d], mcu_y[%d], win_w[%d], win_h[%d] tft.height(%d) tft.width(%d)",
													mcu_x, mcu_y, win_w, win_h, tft.width(), tft.height());
			Serial.println(log);
