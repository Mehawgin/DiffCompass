...
char *color_name[] = { "BLUE", "GREEN", "RED", "WHITE" ,"CYAN","MAGENTA"};
...
void show_chinese_test(void)
{
    int16_t i;
     ...
     show_chinese(15, 108,GREEN,BLACK,"欢迎您",24,1);
     show_chinese(15, 140,BLUE,BLACK,"欢迎您",32,1);
     for (i = 0; i < my_lcd.Get_Display_Width()-1; i++) 
     {
     ...
     show_chinese(15, 108,GREEN,WHITE,"欢迎您",24,0);
     show_chinese(15, 140,BLUE,WHITE,"欢迎您",32,0);
}

void show_pic(void)
{
    int i;
    my_lcd.Set_Addr_Window(my_lcd.Get_Display_Width()-60, 60, my_lcd.Get_Display_Width()-21, 99); 
...
void color_test()
{
  ...
  for(i=0;i<cnum;i++)
  {
    for(n=0;n<30;n++)
    {
      ...
      my_lcd.Fill_Rectangle(n*my_lcd.Get_Display_Width()/30,(my_lcd.Get_Display_Height()/cnum)*i,(n+1)*my_lcd.Get_Display_Width()/30-1,(my_lcd.Get_Display_Height()/cnum)*(i+1)-1);
     }
     show_string(color_name[i],70,((my_lcd.Get_Display_Height()/cnum)-16)/2+(my_lcd.Get_Display_Height()/cnum)*i,2,color_mask[i], BLACK,1);
  }
  delay(1000);
}
...
void text_test(void)
{
    show_string("Hello World!",50,40,1,RED, BLACK,0);
    my_lcd.Set_Text_colour(YELLOW);
    my_lcd.Set_Text_Size(2);
    my_lcd.Print_Number_Float(01234.56789, 4, 50, 48, '.', 0, ' '); 
    show_string("ABC123",50,64,3,BLUE, BLACK,0);
    show_string("Good",50,88,4,GREEN, BLACK,0);
    show_string("By utilizing breath",50,116,1,GREEN, BLACK,0);
    show_string("we soft experiences.",50,124,1,GREEN, BLACK,0);
}
...
void loop() 
{
    ...
    for(rotation = 0;rotation<4;rotation++)
    {
        ...
        for(n=0;n<32;n++)
        {
          ...
          my_lcd.Fill_Rectangle(n*my_lcd.Get_Display_Width()/32,88,(n+1)*my_lcd.Get_Display_Width()/32,115);
        }
        show_string("COLOR TEST",50,95,2,color_mask[rotation+1], BLACK,1);     
        show_pic();
        show_string("SCROLL UP",50,132,2,YELLOW, BLACK,0); 
      ...
      //  }
        ...
        {
            max_scroll = my_lcd.Get_Display_Width()-1;
         }
         else
         {
            max_scroll = my_lcd.Get_Display_Height()-1;
         }
         for(i=0;i<=max_scroll;i++)
         {
          ...
          show_string("SCROLL DN",50,132,2,GREEN, BLACK,0); 
         for(i=0;i<=max_scroll;i++)
         {
          ...
          delay(1000);
          my_lcd.Vert_Scroll(0, max_scroll, 0);
          show_string("SCROLL",50,148,2,YELLOW, BLACK,0); 
